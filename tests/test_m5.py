...
class TestRepeatCommand:
    def test_repeat_sends_previous_apdu(self, capsys):
        ...
        session = MagicMock()
        session.last_apdu = b"\x00\xA4\x04\x00\x00"
        session.last_apdu_label = "00A4040000"
        session.transport.send_apdu.return_value = (b"\x00" * 10, 0x9000)

        cmd_repeat("", session)
        assert session.transport.send_apdu.called
    ...
    def test_repeat_with_count(self, capsys):
        ...
        cmd_repeat("3", session)
        assert session.transport.send_apdu.call_count == 3

    def test_repeat_no_previous(self, capsys):
        """没有上一条 APDU 时提示。"""
        from scsh.commands.apdu import cmd_repeat
        session = MagicMock()
        session.last_apdu = None
        cmd_repeat("", session)
        captured = capsys.readouterr()
    ...
    def test_repeat_not_connected(self, capsys):
        ...
        session = MagicMock()
        session.last_apdu = b"\x00\xA4\x04\x00\x00"
        session.transport.send_apdu.side_effect = Exception("卡片未连接")
        cmd_repeat("", session)
        captured = capsys.readouterr()
...
class TestTimingCommand:
    def test_timing_toggle_on(self, capsys):
        ...
        cmd_timing("on", session)
        assert session.timing_enabled is True
        captured = capsys.readouterr()
    ...
    def test_timing_toggle_off(self, capsys):
        ...
        cmd_timing("off", session)
        assert session.timing_enabled is False
        captured = capsys.readouterr()
...
class TestRecordCommand:
    def test_record_session(self):
        ...
        m = mock_open()
        with patch("builtins.open", m):
            cmd_record("/tmp/session.scsh", session)
